  ...
  Rectangle {
    ...
    GridLayout {
      ...
      Item {
        ...
        Layout.preferredHeight: compactMode ? implicitHeight : capsuleHeight
        Layout.alignment: isVertical ? Qt.AlignHCenter : Qt.AlignVCenter
        visible: showGpuUsage && SystemStatService.gpuAvailable && SystemStatService.gpuUsageAvailable
      ...
      // GPU VRAM Component (percent text + mini gauge — absolute GB lives in the tooltip)
      Item {
        id: gpuVramContainer
        implicitWidth: gpuVramContent.implicitWidth
        implicitHeight: gpuVramContent.implicitHeight
        Layout.preferredWidth: isVertical ? root.width : implicitWidth
        Layout.preferredHeight: compactMode ? implicitHeight : capsuleHeight
        Layout.alignment: isVertical ? Qt.AlignHCenter : Qt.AlignVCenter
        visible: showGpuVram && SystemStatService.gpuAvailable && SystemStatService.gpuVramAvailable
        ...
        GridLayout {
          ...
          NText {
            visible: !compactMode
            text: `${Math.round(SystemStatService.gpuVramPercent)}%`.padStart(paddingPercent, " ")
            family: fontFamily
            pointSize: barFontSize
            applyUiScale: false
            Layout.alignment: Qt.AlignCenter
